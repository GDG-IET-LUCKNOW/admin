...
export const CommunityStoriesPage = () => {
  ...
  const openAddModal = () => {
    setEditingId(null);
    setFormData({ name: "", domain: "", quote: "", github: "", linkedin: "", priority: 1 });
    setIsModalOpen(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await api.put(`/api/community/${editingId}`, formData);
      } else {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <div>
          <TextScramble as="h1" className="text-3xl font-bold tracking-tight">Community Stories</TextScramble>
          <p className="text-foreground/70 mt-1">Manage platform testimonials and success stories.</p>
        </div>
        <button 
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-glass-border bg-white/5">
                ...
                <th className="p-4 font-medium text-foreground/70">Domain/Role</th>
                <th className="p-4 font-medium text-foreground/70">Quote Preview</th>
            ...
            <tbody className="divide-y divide-white/5">
              {stories.length === 0 ? (
                <tr>
                  <td colSpan={4} className="p-8 text-center text-foreground/50">No community stories found. Start by adding one.</td>
                </tr>
              ) : (
                <AnimatePresence>
                  {stories.map((story, idx) => (
                    ...
                    >
                      ...
                      <td className="p-4 text-foreground/70 font-semibold">{story.domain}</td>
                      <td className="p-4 text-foreground/70 truncate max-w-xs">{story.quote}</td>
                      <td className="p-4 text-right">
                        <div className="flex justify-end space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => openEditModal(story)} className="p-2 bg-blue-500/10 text-blue-500 hover:bg-blue-500/20 rounded-lg transition-colors">
                            <Pencil className="w-4 h-4" />
                          </button>
                          <button onClick={() => handleDelete(story._id)} className="p-2 bg-red-500/10 text-red-500 hover:bg-red-500/20 rounded-lg transition-colors">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm overflow-y-auto">
          <div className="bg-background border border-glass-border rounded-2xl w-full max-w-lg shadow-2xl shrink-0 my-auto">
            <div className="p-6 border-b border-glass-border flex justify-between items-center">
              <h2 className="text-xl font-bold">{editingId ? "Edit Story" : "Add Story"}</h2>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1.5 text-foreground/80">Author Name</label>
                ...
                <div>
                  <label className="block text-sm font-medium mb-1.5 text-foreground/80">Domain / Role</label>
                  <input required value={formData.domain} onChange={e => setFormData({...formData, domain: e.target.value})} className="w-full px-4 py-3 bg-foreground/5 border border-glass-border text-foreground rounded-xl focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all placeholder:text-foreground/40 placeholder:font-normal font-medium text-sm" placeholder="e.g. Web Development" />
                </div>
              </div>
              
              <div>
                <label className="block text-sm font-medium mb-1.5 text-foreground/80">Testimonial Quote</label>
                <textarea required rows={4} value={formData.quote} onChange={e => setFormData({...formData, quote: e.target.value})} className="w-full px-4 py-3 bg-foreground/5 border border-glass-border text-foreground rounded-xl focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all placeholder:text-foreground/40 placeholder:font-normal font-medium text-sm resize-none" placeholder="Share your experience with IETECH..." />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1.5 text-foreground/80">Github URL (Optional)</label>
                  <input value={formData.github} onChange={e => setFormData({...formData, github: e.target.value})} className="w-full px-4 py-3 bg-foreground/5 border border-glass-border text-foreground rounded-xl focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all placeholder:text-foreground/40 placeholder:font-normal font-medium text-sm" placeholder="https://github.com/..." />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1.5 text-foreground/80">LinkedIn URL (Optional)</label>
              ...
              <div className="pt-4 flex justify-end space-x-3">
                ...
                <button type="submit" disabled={saving} className="px-6 py-2 bg-primary hover:bg-primary/90 text-primary-foreground rounded-lg font-medium transition-colors flex items-center shadow-lg shadow-primary/20">
                  {saving ? <Loader className="w-4 h-4 animate-spin" /> : <span>{editingId ? "Update" : "Add Story"}</span>}
                </button>
              </div>
            </form>
          </div>
